# This Makefile is for the chr extension to perl.
#
...
#     NAME => q[chr]
#     NO_META => q[1]
#     PREREQ_PM => { Test::More=>q[0], FindBin=>q[0.01], ExtUtils::MakeMaker=>q[6.59], Git::Wrapper=>q[0.01], local::lib=>q[0.01], Pod::Select=>q[0.01], Module::Install=>q[0], Cwd=>q[0.01], Time::Local=>q[0.01], Env=>q[0.01] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.01]
...
chr.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"chr\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <AUTHOR>Sholto Maud &lt;sholto.maud@gmail.com&gt;</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <PERLCORE VERSION=\"5,006,0,0\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Cwd::\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Env::\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"FindBin::\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"0.01\" NAME=\"Git::Wrapper\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Pod::Select\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
...
upgradedeps ::
	$(PERL) Makefile.PL --config= --upgradedeps=Test::More,0,Git::Wrapper,0.01,Time::Local,0.01,local::lib,0.01,Env,0.01,Pod::Select,0.01,Cwd,0.01,FindBin,0.01

upgradedeps_notest ::
	$(PERL) Makefile.PL --config=notest,1 --upgradedeps=Test::More,0,Git::Wrapper,0.01,Time::Local,0.01,local::lib,0.01,Env,0.01,Pod::Select,0.01,Cwd,0.01,FindBin,0.01

listdeps ::
	@$(PERL) -le "print for @ARGV" 

listalldeps ::
	@$(PERL) -le "print for @ARGV" Test::More Git::Wrapper Time::Local local::lib Env Pod::Select Cwd FindBin
